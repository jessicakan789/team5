...
class NewUser(User):
    ...
    _existing_users = get_existing_users()
...
class ExistingUser(User):
    def login(self):
        if self.username in get_existing_users():
...
def start_page():
    print("Welcome to the COVID Calculator")
    while True:
        is_existing = input("Please type 1 if you already have an account, or 2 if you would like to register").strip()
        if is_existing == '1' :
